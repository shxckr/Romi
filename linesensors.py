...
class LineSensors:
    def __init__(self, pin1, pin2, pin3, pin4, pin5, pin6, pin7, samples=4):
        self.adcs = [
            pyb.ADC(pyb.Pin(pin1)),
            pyb.ADC(pyb.Pin(pin2)),
            pyb.ADC(pyb.Pin(pin3)),
            pyb.ADC(pyb.Pin(pin4)),
            pyb.ADC(pyb.Pin(pin5)),
            pyb.ADC(pyb.Pin(pin6)),
            pyb.ADC(pyb.Pin(pin7)),
        ]
    ...
    def read_normalized(self):
        ...
        return [self._normalize(raw[i], self.mins[i], self.maxs[i]) for i in range(7)]
    ...
    def line_error(self, line_is_dark=True):
        ...
        pos_weights = [(i - 3) / 3.0 for i in range(7)]
    ...
    # pins = ['PA4', 'PB0', 'PB1', 'PC0', 'PC1', 'PC2', 'PC3']